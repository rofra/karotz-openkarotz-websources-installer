...
social.twitter.sendSign = function(method, url, params){
    ...
    if( method=="POST" ){
	    for (var i =0; i< keys.length; i++){ 
            log("key=" + keys[i] + " / val=" + params[keys[i]]); 
...
social.twitter.GetHomeTimeline = function(nbMaxMessage, sinceId){
    ...
    var result = social.twitter.sendSign("GET", "https://api.twitter.com/1.1/statuses/home_timeline.json", params);
    log("result:" + result);
    if(result)
...
social.twitter.GetMentions = function(nbMaxMessage, sinceId){
    ...
    var result = social.twitter.sendSign("GET", "https://api.twitter.com/1.1/statuses/mentions.json", params);
    log("result:" + result);
    if(result)
        return JSON.parse(result);
    return false;
...
social.twitter.GetDirectMessages = function(nbMaxMessage, sinceId){
    ...
    var result = social.twitter.sendSign("GET", "http://api.twitter.com/1.1/direct_messages.json", params);
    log("result:" + result);
    if(result)
        return JSON.parse(result);
    return false;
...
social.twitter.GetSearch = function(query, sinceId){
    ...
    var result = social.twitter.sendSign("GET", "https://search.twitter.com/search.json", params);
    log("result:" + result);
    if(result)
        return JSON.parse(result);
    return false;
...
social.twitter.sendMsg = function(text){
    var params = {};
    params.status = text;
    var result = social.twitter.sendSign("POST", "https://api.twitter.com/1.1/statuses/update.json", params);
...
social.twitter.sendDirectMsg = function(screen_name, user_id, text){
    ...
    params.text = text;
    var result = social.twitter.sendSign("POST", "https://api.twitter.com/1.1/direct_messages/new.json", params);
    if(result)
        return true;
    return false;
}
