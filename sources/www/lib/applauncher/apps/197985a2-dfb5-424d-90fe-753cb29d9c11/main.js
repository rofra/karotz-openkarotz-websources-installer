...
var pause = true;
var station = "daimarrage de la radio vivaciter liaige";
var buttonListener = function(event)
{
  if (event == "SIMPLE") {//appui simple permet de mettre en pause / reprendre la lecture du flux radio
    pause = !pause;
    if (!pause) {
      karotz.multimedia.pause();//je mets le lecteur multimedia en pause et je vais faire la sieste
    }
    else {
      karotz.multimedia.resume();//je reprends ma lecture et je me verse une moresque avec plein de sirop d'orgeat
    }
  }
  if (event == "DOUBLE") {
...
var go_light = function() {
    ...
    go_light();
    var path = "http://vivaciteliege.ice.rtbf.be/vivaciteliege.mp3" ;
