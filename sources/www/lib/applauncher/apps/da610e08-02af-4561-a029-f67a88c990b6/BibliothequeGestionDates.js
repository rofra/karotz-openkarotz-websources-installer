...
// *  constituerMessage, finaliserMessage, rechercheProchainsJours, CalculIndiceJour, ProchaineDateSuivantJourMois                                                    *
// *                                                                                                                                                                  *
// *  Historique :                                                                                                                                                    *
// *  0.0.1   12 fonctions construite à partir du main de 01-app_Poubelles v0.1.9                                                                                     *
// *  0.0.3   Modification de prochaineDate et prochaineDateSuivantPeriode (prise en compte de 1erMardiMois)nouvelles fcts : CalculIndiceJour, ProchaineDateSuivantJourMois*
// *  0.0.4   reprise des modifications de la 0.0.2 collecte ou rappel qui modifient les fonctions :rechercheProchainsJours,finaliserMessage,constituerMessage        *
// ********************************************************************************************************************************************************************
...
// ********************************************************************************************************************************************
// * Nom fonction : conversionDatestrToDate  modifiée dans la version 0.0.3                                                                   * 
...
// ********************************************************************************************************************************************
// * Nom fonction : CalculIndiceJour  arrivée dans la version 0.0.3                                                                           * 
...
// ********************************************************************************************************************************************
// * Nom fonction : ProchaineDateSuivantJourMois  arrivée dans la version 0.0.3                                                               * 
...
var prochaineDateSuivantPeriode = function (dDateDebut, bDateEgale, sListeDates, sSoirOuMatin) {
  ...
  var datei=conversionDatestrToDate (tableauPeriode[1]);
...
var prochaineDateListeDates = function (dDateDebut, bDateEgale, sListeDates, sSoirOuMatin) {
  ...
  while  (!bDateTrouvee && i<tableauDates.length) {
    var datei=conversionDatestrToDate (tableauDates[i]);
    if ((bDateEgale && datei.toLocaleString() == dDateDebut.toLocaleString()) || (!bDateEgale && datei>= dDateDebut)) {
      if (datei.toLocaleString() == dDateDuJour.toLocaleString()) {
	if (sSoirOuMatin!="") {sProchaineDateStr = "ce "+ sSoirOuMatin;} else {sProchaineDateStr = "aujourdui";}
      }
...
// Renvoie la prochaine date de collecte ou rappel correspondant  soit à un jours de la semaine soit à une liste de dates (si sJours="liste dates")
var prochainJour = function (dDateDebut, bDateEgale, sJours, sListeDates, sSoirOuMatin) {
  ...
  if (bRes) {return prochaineDate (dDateDebut, bDateEgale, sListeDates, sSoirOuMatin);}
    ...
    //on cherche le prochain jour de la semaine qui est dans les jours de collecte ou rappel
...
// ZZZ fonction qui construit les message (pour SModeAnnonce=NJoursProchains) pour chaque jour suivant le jour trouvé pour une collecte ou un rappel
var constituerMessage = function(sMomentTrouve, sRappel, sCollecteOuRappel) {
  var i=0;
  var bTrouve =false;
  while (!bTrouve && i<tProchainsMoments.length) {
   if (sMomentTrouve==tProchainsMoments[i]) {
     if (sCollecteOuRappel=='Rappel') {tMessageNMoments[i]=tMessageNMoments[i] + " " + sRappel +"," ;}
     else {tMessageNMoments[i]=tMessageNMoments[i] + " "+sCollecteOuRappel+" " + sRappel +"," ;}
     bTrouve= true;
   }
   i=i+1;
  }
...
// ZZZ fonction qui finalise un Message  (pour SModeAnnonce=NJoursProchains) à partir de tous les messages constitués pour chaque moment (ce matin, ce soir, etc.)
var finaliserMessage = function (sCollecteOuRappel) {
  ...
  if (sMessageConstruit=="") { sMessageComplet = "Il n'y a pas de "+sCollecteOuRappel+" pour aujourdui et les "+ parseInt(sModeAnnonce)+" prochains jours";}
  else {sMessageComplet = "Vos "+sCollecteOuRappel+" pour aujourdui et les "+ parseInt(sModeAnnonce)+" prochains jours sont :"+sMessageConstruit;}
  return sMessageComplet;
}

// ZZZ+ fonction qui recherche pour les prochains jours les collectes et les rappels et qui constitue les messages correspondant (utilisé dans le mode sAnnonce=NprochainsJours)
var rechercheProchainsJours = function (iNombreJoursMaximum, tTypes, tListeDates, tJours, tMatinOuSoir, sCollecteOuRappel,tDate) {
  var dateDebut = new Date();
  var bDateEgale = true;
      
  definitionNProchainsMoments(iNombreJoursMaximum, sCollecteOuRappel);
  ...
  while (iNombreJoursMaximum >=0) {
    ...
    debuglog("recherch collecte ou rappel pour dateDebut :"+dateDebut.toLocaleString()+"; dDateDuJour :"+dDateDuJour.toLocaleString());
    // attention il peut y avoir une heure de décalage sur la date calculée ci-dessous avec le passage heure hiver/été prise en compte parsetTime
    // dateDebut.setTime(dDateDuJour.getTime() + iNombreJoursMaximum * 24 * 3600 * 1000);
    for (var i=0; i<tTypes.length; i++) {
      if (tTypes[i] != 'aucune') {
	//regarder si le prochain jour datedebut est une date de collecte ou rappel
	tDate[i] = prochainJour (dateDebut, bDateEgale, tJours[i], tListeDates[i], tMatinOuSoir[i]); 
	// regarder si le jour retourné est compris dans les Nprochains Jours et préparer le message
	constituerMessage(tDate[i], tTypes[i], sCollecteOuRappel);
      }
     }
     iNombreJoursMaximum=iNombreJoursMaximum-1;
  }
  return finaliserMessage(sCollecteOuRappel); 
}