StateMachine = {
  ...
  create: function(cfg) {
    ...
    fsm.can     = function(event) { return !!(events[event] && events[event][this.current]); };
    fsm.cannot  = function(event) { return !this.can(event); };
    fsm.getEvents  = function() { 
        var res = [];
        for(event in events)
            if(fsm.can(event)) res.push(event)
        return res;
    };
    
    fsm.genGraph = function() {
        var res = "digraph G {\n";
        var len=cfg.events.length;
        for(var i=0; i<len; i++)
            res += "\t" + cfg.events[i].from + " -> " + cfg.events[i].to + ' [label="' + cfg.events[i].name + "\"];\n";
        res +="}\n"
        return res;
    }
