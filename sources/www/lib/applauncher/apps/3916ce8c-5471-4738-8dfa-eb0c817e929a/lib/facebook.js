...
social.facebook.getMail = function(timestamp){
  ...
  var messages = social.facebook.FQL("SELECT message_id,thread_id,author_id,body,created_time,attachment FROM message WHERE created_time > "+ timestamp +" AND thread_id IN ("+threadid+") ORDER BY created_time DESC LIMIT 20")
...
social.facebook.getMail = function(timestamp){
  ...
  var result = new Array(thread.length);
  ...
  for(i=0;i<thread.length;i++){
    //threadid += ","+ thread[i].thread_id 
    log("# thread "+ i)
    log("# thread[i].unread "+ thread[i].unread)
    result[i] = {};
    result[i].messages = new Array(thread[i].unread);
    ...
    var uidList = messages[0].author_id
    for(j=1;j<messages.length;j++){
      uidList += ","+ messages[j].author_id 
    }
    
    for(j=(messages.length -1); j > (messages.length -1 - thread[i].unread); j-- ){
        log("# thread[i].messages " + i + " " + result[i].messages)
        index = j - (messages.length - thread[i].unread)
        log("# index :  " + index)
        result[i].messages[index] = messages[j];
    }
