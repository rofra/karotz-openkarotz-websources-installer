...
social.facebook.getStream = function(timestamp){
  ...
  var ret= social.facebook.FQL("SELECT post_id,app_id,updated_time,created_time,attribution,actor_id,target_id,message,app_data,action_links,attachment,comments,likes FROM stream WHERE filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') AND is_hidden = 0 AND created_time > "+ timestamp +" LIMIT 50")
...
social.facebook.getWall = function(timestamp){
  if (!timestamp || timestamp == "") timestamp = 0;
  var ret= social.facebook.FQL("SELECT post_id,app_id,updated_time,created_time,attribution,actor_id,target_id,message,app_data,action_links,attachment,comments,likes FROM stream WHERE filter_key = 'others' AND source_id = me() AND is_hidden = 0 AND created_time > "+ timestamp +" LIMIT 50")
...
social.facebook.getMail = function(timestamp){
  ...
  var thread = social.facebook.FQL("SELECT thread_id,subject,snippet,snippet_author,updated_time,unread FROM thread WHERE folder_id = 0 and updated_time > "+ timestamp +" LIMIT 30")
  log("thread "+JSON.stringify(thread))
  if (!thread.length || thread.error_code ){
    thread.error_code = 1;
    return thread;
  } 
  ...
  if (!messages.length || messages.error_code ){
    messages.error_code = 1;
    return messages;
  }
...
social.facebook.getNotification = function(timestamp){
  ...
  var notif = social.facebook.FQL("SELECT created_time,notification_id, sender_id, title_text, body_text, href FROM notification WHERE recipient_id=me() AND created_time > "+ timestamp )
