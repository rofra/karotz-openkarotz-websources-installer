...
social.facebook.getStream = function(timestamp){
  ...
  var ret= social.facebook.FQL("SELECT post_id,app_id,updated_time,created_time,attribution,actor_id,target_id,message,app_data,action_links,attachment,comments,likes FROM stream WHERE filter_key in (SELECT filter_key FROM stream_filter WHERE uid=me() AND type='newsfeed') AND actor_id<>me() AND is_hidden = 0 AND created_time > "+ timestamp +" LIMIT 50")
