...
cleanToTts.specSymbTxt = new Array();
cleanToTts.specSymbTxt["fr"] = " , ";
cleanToTts.specSymbTxt["en"] = " , ";
cleanToTts.specSymbTxt["de"] = " , ";
cleanToTts.specSymbTxt["es"] = " ";
...
social.twitter.sendSign = function(method, url, params){
	...
	if( method=="GET" ){
	    ...
	    result = http.get(url + "?" + data2 + "&oauth_signature=" + encodeURIComponent(sign), {});
	    delete data
	    delete data2
	    delete params
	    delete keys
	    delete oauth_nonce
	    delete oauth_timestamp
	    delete sign
		return result;
	}
...
social.twitter.getProfile = function(){
    var params = {};
    params.skip_status =true;
    var result = social.twitter.sendSign("GET", "https://api.twitter.com/1/account/verify_credentials.json", params);
    log("result:" + result);
...
social.twitter.cleanText = function(text, lang){
    text = text.replace(social.twitter.specSymb, cleanToTts.specSymbTxt[lang]);
