...
function automate(i)
{
	...
	else
	{
		...
		var regexp=new RegExp("Voter", "g");
		description=description.split(regexp);
		description=description[0];
		speakAndGoto(" <break time='700ms'/>"+description, i+1, 500);
	}
}
...
var onKarotzConnect = function(data)
{
	...
	var data = http.get("http://feeds.feedburner.com/Pebkacfr?format=xml");

	var objDom = new XMLDoc(data);
	domTree = objDom.docNode;
	speakAndGoto("PEBKAC", 1, 0);
}
