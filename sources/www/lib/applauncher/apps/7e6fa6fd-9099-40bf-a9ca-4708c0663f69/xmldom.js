...
function XMLNode(nodeType,doc, str) {
    ...
    *********************************************************************************************************************/
    ...
    if (nodeType == 'TEXT' || nodeType == 'CDATA' || nodeType == 'COMMENT') {
        this.content = str;//log('c est text ou cdata ou comment' + str)
    }
    else {//log('ca n est rien de cela');
        this.content = null;
    }
...
function _XMLNode_getElementById(id) {
    ...
    ************************************************************************************/
    ...
    //log("tag name=" + node.tagName);
    //log("id=" + node.getAttribute("id"));
    if (node.getAttribute("id") == id) {
        return node;
    }
    else{
        var elements = node.getElements();
        //log("length=" + rugrats.length);
        var intLoop = 0;
...
function _XMLNode_getText() {
    ...
    *********************************************************************************************************************/
    ...
    if (this.nodeType=='ELEMENT') {
        if (this.children==null) {//log("Element");
            return null;
        }
        var str = "";
        for (var i = 0; i < this.children.length; i++) {
